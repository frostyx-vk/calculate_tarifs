...
credential.addEventListener('input', function(){
  if (credential.checked) {
    window.location.href = '../html/calculate_order.html'
  };
  if (credential.checked === false) {
    window.location.href = '../html/without-enroll.html'
  }
})
...
function calculateWithEnrollWithoutDiscount() {
  ...
  for (const inputRadioGrade of inputsRadioGrade) {
    if (inputRadioGrade.checked) {
      totalPriceWithoutDiscount = totalPriceWithoutDiscount * inputRadioGrade.dataset.value;
    };
  };
